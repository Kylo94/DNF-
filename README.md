...
- 「导出 Excel」在登记页和排表页都能点，导出的都是同一个完整文件（`DNF打团排表_时间戳.xlsx`）
- 「导入 Excel」两个页面也都能点：自动识别两个 sheet，角色按「玩家 + 角色称呼」对应回登记表；同一个文件导回去能完整还原角色与每一波的排表
...
- **页面布局**：顶部工具条 → 队伍统一配置 → **全部波次（每行一波，一行里红 / 黄 / 绿三组人员一屏看全）** → 未登场角色
